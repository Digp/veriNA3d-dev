...
filled in 2006 by the R package bio3d [@Grant2006]. It was presented as a 
suite of tools to handle PDB formated structures, and trajectories. 
It integrates a variety of tools to analyse these data, from 
sequence to 3D structure (RMSD, NMA, PCA... see their reference for details), 
...
blocks (list detailed in the README file):

* Dataset level: Functions to get and analyse lists of pdb IDs. This includes
the representative lists of RNA by [@Leontis2012] and analytical tools.

* Structure level: Functions to get data, parse mmCIF files and analyse these
data.

* Plots: A minimum sample of plots to be done after the previous analysis. 


# mmCIF files and the S4 CIF attributes
