...
author:
  - name: Diego Gallego Perez
    affiliation:
    - University of Barcelona - Department of Biochemistry and Molecular 
        Biomedicine
...
data. This includes a wrapper of DSSR [@Lu2015] and a function to calculate
...
The list of functions is **intendedly limited** in comparison with the dozens
of *enpoints* of the REST API. Integrating them all would innecessarily
...
(e.g. a peptide from a complex, or a ligand and it's binding site). For a 
...
of two entities) and `measureElenoDist` (returns all the distances between a
set of atoms).
...
VeriNA3d integrates
the function `getLeontisList`, which returns the desired non-redundant list
of RNA structures, as found in their 
[website](http://rna.bgsu.edu/rna3dhub/nrlist/) [@Leontis2012]. 
...
```{r getLeontisList(release, threshold)}
...
When analysing a dataset, a particular piece of data might come useful, such
...
## Reproducing eta-theta Ramachandran-like plots

In 2007, Pyle group published a new metric to study the RNA backbone
in a simplified way [@Wadley2007]. The parameters of study were two
pseudo-torsional angles called eta and theta. When plotted, they resemble
the Ramachandran plots for proteins. With veriNA3d, these plots can be
esasily reproduced.

For the sake of this example, their dataset was integrated in the package and
can be accessed with the following command.

```{r data(dataset_wadley2007)}
data(dataset_wadley2007)
head(dataset_wadley2007)
```
Then, the function `pipeNucData` can extract multiple structural parameters
for all the dataset, to be then analysed afterwards.

```{r pipeNucData(pdblist, chainlist, cores), eval=FALSE}
...
(e.g. ".cif.gz", ".cif" or ".pdb"). The following lines have not been executed:
