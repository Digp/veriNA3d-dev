#' Obtain measurement data for nucleotides 
#' 
...
function(distances) {
    ...
    if (!is.null(distances) && !is.na(distances) && distances == "default") {
        distances <- .distances
...
function(angles) {
    ...
    if (!is.null(angles) && !is.na(angles) && angles == "default") {
        angles <- .angles
...
function(.index, .reslist, .inslist, .ridlist, .pdb, 
            .distances, .angles, .torsionals, .v_shifted, .Dp, .pucker) {
