...
#   class is a data.frame where each column is a list of chains with the
#  Leontis format (pdbID|model|chain)
#   dir is the string with the name of the desired column

#OUTPUT: returns a vector containing the ntID of the desired nucleotides 
#according with the selected column/list of chains
...
#    listpdb<-as.data.frame(matrix(unlist(strsplit(listpdb,split="|",
#fixed=TRUE)),ncol=3,byrow=TRUE),stringsAsFactors=FALSE)
