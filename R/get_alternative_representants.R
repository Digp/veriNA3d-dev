...
#'
#' This function is closely related with get_leontis_nr_list(). From its 
...
#'
#' @param data The output of get_leontis_nr_list.
...
#'
#' @examples 
#'  data <- get_leontis_nr_list( release = 3.2, threshold = "1.5A" )
#'  alternative <- get_alternative_representants( data = data, 
#'          type = "nakedRNA")
#'
...
function(   eqclass, members,
    ...
    if(is.null(technique)) { Tech <- ""; technique <- "" }
    if(is.null(resol)) { Resol <- ""; resol <- " " }
    if(is.null(type)) { Type <- ""; type <- "" }
    out <- NA

    for(i in 1:length(Members)) {
#Save pdbID instead of Leontis format
        pdbID <- substr(Members[i], 1, 4)
    if(verbose) cat(paste(pdbID, " ... ", sep = ""))

#If interested in any technique, query experimental technique and cache result
        if(technique != "") {
...
#If a particular type of RNA is specified, query and cache
        if(type != ""){
        Type <- RNA_classifier(pdbID, reuse = T, 
                   #verbose = verbose, 
                   envir = parent.frame(n = 1))
    }
